...
/**
 * Abstract class for builders producing components with buttons.
 */
export abstract class ButtonHoldingBuilder<T> extends Builder<T> implements ButtonHolder {
    ...
    public addExtensionButton(title: string, extension: ChatExtension, options?: {
        ...
        if (options) {
            ...
            if (button.messenger_extensions) {
                // prepare correct using of messenger extension (https://developers.facebook.com/docs/messenger-platform/webview/extensions)
                // whitelist domain
            }

            // not implemented - BotUtils needs accessToken
            */
        }
